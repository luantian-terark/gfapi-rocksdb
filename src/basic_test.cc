...
namespace
{
    ...
    std::unique_ptr<Env> def_env;// (new NormalizingEnvWrapper());
    ...
    std::unique_ptr<Env> mock_env;// (new MockEnv(new GlusterEnv("192.168.0.109", "test-vol")));
    INSTANTIATE_TEST_CASE_P(MockEnv, EnvBasicTestWithParam,
        ::testing::Values(mock_env.get()));

#ifndef ROCKSDB_LITE
    std::unique_ptr<Env> mem_env;// (NewMemEnv(new GlusterEnv("192.168.0.109", "test-vol")));
...
int main(int argc, char** argv) {
    if (argc != 3) {
        printf("expect args: %s <host> <volume>\n", argv[0]);
        exit(0);
    }
    unique_ptr<Env> testing_env{ new GlusterEnv(argv[1], argv[2]) };
    def_env.reset(new NormalizingEnvWrapper(testing_env.get()));
    mock_env.reset(new NormalizingEnvWrapper(testing_env.get()));
    mem_env.reset(new NormalizingEnvWrapper(testing_env.get()));
    ::testing::InitGoogleTest(&argc, argv);
    return RUN_ALL_TESTS();
}
// copy from testharness.cc
namespace rocksdb {
    namespace test {
